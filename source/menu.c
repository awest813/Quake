...
{
    Draw_Character(cx + ((spacing_x_res) >> 1), line, num);
}
...
{
    Draw_TransPic(x + ((spacing_x_res) >> 1), y, pic);
}

void
M_DrawPic(int x, int y, const qpic8_t *pic)
{
    Draw_Pic(x + ((spacing_x_res) >> 1), y, pic);
}
...
{
    Draw_TransPicTranslate(x + ((spacing_x_res) >> 1), y, pic,
			   translationTable);
}
#endif /* NQ_HACK */
